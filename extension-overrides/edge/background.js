...
// [Edge workaround] Edge has a bug where `chrome.runtime.sendMessage()` always
// returns `undefined`, so the content script replaces it with a port-based
// implementation. This listener handles those port-based messages, routing
// responses back via `port.postMessage()` with the original `requestId`.
chrome.runtime.onConnect.addListener((port) => {
  port.onMessage.addListener((message) => {
    const { requestId, ...rest } = message;
    const sendResponse = (data) => port.postMessage({ requestId, data });
    handleMessage(rest, port.sender, sendResponse);
  });
});
