...
// returns `undefined`. We fix this by replacing it with a port-based
// implementation that behaves identically. This block is the only difference
// between this file and the non-Edge version.
const _port = chrome.runtime.connect();
const _pendingRequests = {};
let _nextRequestId = 0;

_port.onMessage.addListener((response) => {
  const resolve = _pendingRequests[response.requestId];
  if (resolve) {
...
chrome.runtime.sendMessage = (message) => {
  return new Promise((resolve) => {
    ...
    _port.postMessage({ ...message, requestId });
  });
};
